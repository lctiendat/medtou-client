...
const HeathFacility: React.FC = () => {
  ...
  useEffect(() => {
    if (window.location.pathname == '/healthfacilitylist') {
      checkPermission();
      if (mapRef.current) {
  ...
  const getDataByPosition = async () => {
    if (scope && currentPosition.lat !== 0 && currentPosition.lng !== 0) {
      ...
      try {
        ...
        if (amenity === 'all') {
          urls = [
            `https://overpass-api.de/api/interpreter?data=[out:json];node[amenity=pharmacy](around:${Number(scope) * 1000},${currentPosition.lat},${currentPosition.lng});out body;`,
            `https://overpass-api.de/api/interpreter?data=[out:json];node[amenity=hospital](around:${Number(scope) * 1000},${currentPosition.lat},${currentPosition.lng});out body;`
          ];
        } else {
          urls = [
        ...
        <div className="min-h-screen bg-gray-100 flex flex-col">
          ...
          <div className="p-4 bg-red-50">
            ...
            <div className="flex space-x-2 overflow-x-auto pb-4">
              <button
                className={`py-2 px-4 rounded-lg whitespace-nowrap ${amenity === 'all' ? 'bg-red-700 text-white' : 'bg-red-500 text-white'}`}
                onClick={() => handleFilterClick('all')}
              >
                Tất cả
              </button>
              <button
                className={`py-2 px-4 rounded-lg whitespace-nowrap ${amenity === 'pharmacy' ? 'bg-red-700 text-white' : 'bg-red-500 text-white'}`}
                onClick={() => handleFilterClick('pharmacy')}
              >
                Nhà thuốc
              </button>
              <button
                className={`py-2 px-4 rounded-lg whitespace-nowrap ${amenity === 'hospital' ? 'bg-red-700 text-white' : 'bg-red-500 text-white'}`}
